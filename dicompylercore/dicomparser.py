...
if __name__ == '__main__':
    dp = DicomParser("H:\\3DslicerData\\HMR\\002\\CTSTR\\rt000000.dcm")
    structures = dp.GetStructures()
    len = len(structures)

    for i in range(len-1,0,-1):
        # del dp.ds.StructureSetROISequence[i]
        temp = dp.ds.StructureSetROISequence[i]
        print(type(temp.ROIName))
        # print(dp.ds.StructureSetROISequence[i])
        #del dp.ds.RTROIObservationsSequence[i]
        # print(dp.ds.ROIContourSequence[i])
        print(i)

    dp.ds.save_as("./test.dcm",False)
